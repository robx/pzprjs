...
Graphic.prototype = {
	...
	drawDashedGrid : function(x1,y1,x2,y2){
		...
		if(g.canvas){
			...
			g.strokeStyle = this.gridcolor;
			for(var i=xa;i<=xb;i++){ if(this.vnop("bdy_"+i,-1)){
	...
	setVectorFunctions : function(){
		...
		this.vnop = function(vid, isfill){ // 色の変更なし:-1, strokeのみ:0, fillのみ:1, 両方:2
			g.vid = vid;
			if(!!g.elements[vid]){
