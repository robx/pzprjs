//
// パズル固有スクリプト部 あみぼー版 amibo.js v3.3.4
//
Puzzles.amibo = function(){ };
Puzzles.amibo.prototype = {
	setting : function(){
		// グローバル変数の初期設定
	...
	protoChange : function(){
		Cross.prototype.color = "";
	},
	protoOriginal : function(){
		delete Cross.prototype.color;
	},
	...
	graphic_init : function(){
		...
		pc.fontcolor = pc.fontErrcolor = "black";

		pc.fontsizeratio = 0.85;
		pc.circleratio = [0.42, 0.42];

		pc.prepaint = function(x1,y1,x2,y2){
			this.setRange(x1-2,y1-2,x2+2,y2+2);
		...
		pc.getBarColor = function(c,vert){
			...
			else if(k.irowake===0 || !pp.getVal('irowake') || !bd.cell[c].color){ color = this.linecolor;}
		...
		pc.drawTateyokos_sub = function(){
			this.vinc('cell_tateyoko', 'crispEdges'); /* 同じレイヤでよい */
		...
		pc.drawPekeBorder = function(){
			this.vinc('border_pbd', 'crispEdges');
		...
		pc.repaintBars = function(clist){
			var d = ans.getSizeOfClist(clist,f_true);
			this.paintRange(d.x1-2, d.y1-2, d.x2+2, d.y2+2);
		};
	},
	...
	answer_init : function(){
		...
		ans.check1st = function(){ return this.checkOneArea( bd.bars.getAreaInfo() );};

		ans.getBarInfo = function(){
			function eachcell(tc, qa_chk, vert){
				var tc=bd.cnum(bx,by), qa=bd.QaC(tc);
				if(qa===qa_chk||qa===3){
					if(id===null){
						binfo.max++;
			...
			binfo.pole = [];
			for(var c=0;c<bd.cellmax;c++){ binfo.id[c]=[]; binfo.pole[c]=[];}
			for(var bx=bd.minbx+1;bx<=bd.maxbx-1;bx+=2){
				var id=null, tc0=null;
				for(var by=bd.minby+1;by<=bd.maxby-1;by+=2){
					eachcell(bd.cnum(bx,by),1,true);
				}
			}
			for(var by=bd.minby+1;by<=bd.maxby-1;by+=2){
				var id=null, tc0=null;
				for(var bx=bd.minbx+1;bx<=bd.maxbx-1;bx+=2){
					eachcell(bd.cnum(bx,by),2,false);
				}
			}
		...
		ans.checkLineCount = function(binfo, type){
			var result = true;
			for(var c=0;c<bd.cellmax;c++){
		...
		ans.checkPoleLength = function(binfo,type){
			var result=true;
			for(var c=0;c<bd.cellmax;c++){
				...
				for(var i=0,len=binfo.pole[c].length;i<len;i++){
					...
					if((type===1 && llen>qn) || (type===2 && llen<qn)){
						if(this.inAutoCheck){ return false;}
						bd.sErC([c],1);
						this.setErrorBar(binfo.room[id]);
						result = false;
					}
				}
			}
		...
		ans.checkCrossedLength = function(binfo){
			var result=true;
		...
		ans.setErrorBar = function(bardata){
			var clist = bardata.idlist, vert = bardata.vert;
			for(var i=0;i<clist.length;i++){
				var c=clist[i], err=bd.cell[c].error;
				if     (err===4){ /* nop */ }
				else if(err===5){ if(!vert){ bd.sErC([c],4);}}
				else if(err===6){ if( vert){ bd.sErC([c],4);}}
				else{ bd.sErC([c],(vert?5:6));}
			}
		};

		ans.checkLoop = function(){
		...
		ans.searchloop = function(fc, sinfo, sdata){
...
BarManager.prototype = {
	...
	//--------------------------------------------------------------------------------
	// bars.initBarea()  黒マス関連の変数を初期化する
	// bars.resetBarea() 黒マスの情報をresetして、1から割り当てしなおす
	//--------------------------------------------------------------------------------
	initBar : function(){
	...
	//--------------------------------------------------------------------------------
	// area.getAreaInfo()  情報をAreaInfo型のオブジェクトで返す
	//--------------------------------------------------------------------------------
	getAreaInfo : function(block){
		var data = this.barinfo, info = new AreaInfo();
