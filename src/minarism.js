//
// パズル固有スクリプト部 マイナリズム版 minarism.js v3.1.9
//
...
Puzzle.prototype = {
	...
	check : function(){
		...
		if( !this.checkBDnumber() ){
			ans.setAlert('丸付き数字とその両側の数字の差が一致していません。', 'The Difference between two Adjacent cells is not equal to the number on circle.'); return false;
		}

		if( !this.checkBDmark() ){
			ans.setAlert('不等号と数字が矛盾しています。', 'A inequality sign is not correct.'); return false;
		}
