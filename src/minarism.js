...
Encode:{
	...
	decodeMinarism : function(type){
		...
		for(var i=0;i<bstr.length;i++){
			...
			else if(ca==="g"){ tmp = ((type===pzprurl.PZPRV3 || id<bd.qcols*bd.qrows)?1:2);}
			else if(ca==="h"){ tmp = ((type===pzprurl.PZPRV3 || id<bd.qcols*bd.qrows)?2:1);}
			else if(this.include(ca,'i','z')){ id+=(parseInt(ca,36)-18);}
			else if(type===pzprurl.PZPRAPP && ca==="/"){ id=bd.cellmax-1;}
	...
	encodeMinarism : function(type){
		...
		for(var id=0,max=bd.bdmax+(type===pzprurl.PZPRV3?0:bd.qcols);id<max;id++){
			if(type===1){
			...
			if(id<bd.bdmax){
				...
				if     (dir===k.UP||dir===k.LT){ pstr = ((type===pzprurl.PZPRV3 || id<bd.cellmax)?"g":"h");}
				else if(dir===k.DN||dir===k.RT){ pstr = ((type===pzprurl.PZPRV3 || id<bd.cellmax)?"h":"g");}
				else if(qnum===-2){ pstr = ".";}
