//
// パズル固有スクリプト部 黒マスはどこだ版 kurodoko.js v3.1.9
//
...
Puzzle.prototype = {
	...
	pzloutput : function(type){
		...
		else if(type==2){ document.urloutput.ta.value = enc.kanpenbase()+"kurodoko.html?problem="+this.pzldataKanpen();}
	...
	check : function(){
		...
		if( !ans.linkBWarea(warea) ){
			ans.setAlert('白マスが分断されています。','White cells are devided.'); return false;
		}

		if( !this.checkCellNumber(warea) ){
			ans.setAlert('数字と黒マスにぶつかるまでの4方向のマスの合計が違います。','The number and the sum of the coutinuous white cells of four direction is different.'); return false;
		}
