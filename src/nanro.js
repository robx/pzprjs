//
// パズル固有スクリプト部 ナンロー版 nanro.js v3.1.9p1
//
...
Puzzle.prototype = {
	...
	check : function(){
		...
		if( !ans.check2x2Block( function(cc){ return (puz.getNum(cc)>0);} ) ){
			ans.setAlert('数字が2x2のかたまりになっています。','There is a 2x2 block of numbers.'); return false;
		}

		if( !ans.checkSideAreaCell(rarea, function(area,c1,c2){ return (puz.getNum(c1)>0 && puz.getNum(c1)==puz.getNum(c2));}, false) ){
			ans.setAlert('同じ数字が境界線を挟んで隣り合っています。','Adjacent blocks have the same number.'); return false;
		}
		...
		if( !this.checkErrorFlag(rarea, 1) ){
			ans.setAlert('入っている数字の数が数字より多いです。','A number is bigger than the size of block.'); return false;
		}
		...
		if( !this.checkErrorFlag(rarea, 2) ){
			ans.setAlert('入っている数字の数が数字より少ないです。','A number is smaller than the size of block.'); return false;
		}
