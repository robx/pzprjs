...
(function(pidlist, classbase) {
	...
	AnsCheck: {
		...
		checkParallelBarCount: function() {
			this.checkRowsColsSeparate(
				this.isParallelCount,
				function(cell) {
					return cell.qans;
				},
				"baParaGe3"
			...
			allloop: do {
				...
				for (var by = 1; by <= bd.maxby; by += 2) {
					for (var bx = 1; bx <= bd.maxbx; bx += 2) {
						var val = categoryfunc(bd.getc(bx, by)),
							tx = bx;
						while (
							tx + 2 < bd.maxbx &&
							categoryfunc(bd.getc(tx + 2, by)) === val
						) {
							tx += 2;
						}
				...
				for (var bx = 1; bx <= bd.maxbx; bx += 2) {
					for (var by = 1; by <= bd.maxby; by += 2) {
						...
						while (
							ty + 2 < bd.maxby &&
							categoryfunc(bd.getc(bx, ty + 2)) === val
						) {
							ty += 2;
						}
		...
		isParallelCount: function(clist, info) {
			if (clist[0].qans === 0) {
				return true;
			} else if (clist[0].qans === 12 && info.isvert) {
				return true;
			} else if (clist[0].qans === 13 && !info.isvert) {
				return true;
		...
		checkMajorityBarOver: function() {
			this.checkMajorityBarCount(true, "bkMajorBarGt");
		},
		checkMajorityBarLack: function() {
			this.checkMajorityBarCount(false, "bkMajorBarLt");
		},
