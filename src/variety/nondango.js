...
MouseEvent:{
	...
	mouseinput_auto : function(){
		if(this.puzzle.playmode){
			...
			else if(this.mouseend && this.notInputted()){ this.inputcircle();}
		}
	},
	...
	inputcell_nondango : function(){
		...
		if(cell.ques===0 || this.inputMode==='shade'){
			...
			if(this.mousestart){ this.inputData = (cell.qsub===0?2:0);}
			cell.setQans(0);
...
Encode:{
	...
	decodeBoard_nondango : function(){
		var bstr = this.outbstr, c=0, i=0, bd = this.board, twi=[16,8,4,2,1];
		allloop:
	...
	encodeBoard_nondango : function(type){
		var cm="", count=0, pass=0, bd = this.board, twi=[16,8,4,2,1];
		for(var c=0;c<bd.cell.length;c++){
			if(bd.cell[c].ques===8){ pass+=twi[count];} count++;
...
FileIO:{
	...
	decodeCellQnumanssub : function(){
		this.decodeCell( function(cell,ca){
			if     (ca==="#"){ cell.ques = 0; cell.qans = 1;}
			else if(ca==="%"){ cell.ques = 0; cell.qsub = 1;}
			else if(ca==="o"){ cell.ques = 0;               }
			else if(ca==="+"){ cell.ques = 8; cell.qsub = 1;}
		});
	},
...
AnsCheck:{
	...
	checkOverShadeCircle : function(){
		this.checkAllBlock(this.board.roommgr, function(cell){ return cell.qans===1;}, function(w,h,a,n){ return (a<=1);}, "bkMSGe2");
	},
	checkNoShadeCircle : function(){
		this.checkAllBlock(this.board.roommgr, function(cell){ return cell.qans===1;}, function(w,h,a,n){ return (a>=1);}, "bkNoMS");
	},

	checkThreeShadedCircles : function(){
		this.checkThreeCircles(function(cell){ return cell.qans===1;}, "msConsecGt3");
	},
	checkThreeUnshadedCircles : function(){
		this.checkThreeCircles(function(cell){ return cell.ques===0 && cell.qans===0;}, "muConsecGt3");
	},
...
FailCode:{
	bkMSGe2 : ["1つの領域に複数のくろまるがあります。","An area has two or more shaded circles."],
	bkNoMS  : ["くろまるのない領域があります。","An area has no circle."],
	msConsecGt3 : ["くろまるが3つ以上連続しています。","There are three or more shaded circles in a row."],
	muConsecGt3 : ["しろまるが3つ以上連続しています。","There are three or more unshaded circles in a row."]
}
}));
