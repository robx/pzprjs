...
(function(pidlist, classbase) {
	...
	"AreaBarGraph:AreaGraphBase": {
		...
		isedgevalidbynodeobj: function(cell1, cell2) {
			...
			if (dir === cell1.UP || dir === cell1.DN) {
				return cell1.qans === 12 && cell2.qans === 12;
			} else if (dir === cell1.LT || dir === cell1.RT) {
				return cell1.qans === 13 && cell2.qans === 13;
			}
			return false;
		}
	},
		...
		decodeTateyoko: function() {
			...
			for (i = 0; i < bstr.length; i++) {
				...
				if (ca === "x") {
					...
					c += parseInt(bstr.charAt(i + 1), 16) - 1;
					i++;
				}
	...
	FileIO: {
		decodeData: function() {
			this.decodeCell(function(cell, ca) {
				if (ca >= "a" && ca <= "f") {
					cell.ques = 1;
					cell.qnum = { a: 1, b: 2, c: 3, d: 4, e: 0, f: -1 }[ca];
				} else if (ca === "?") {
					cell.qnum = -2;
				} else if (ca !== ".") {
					cell.qnum = +ca;
			...
			this.encodeCell(function(cell) {
				if (cell.ques === 1) {
					if (cell.qnum === -1 || cell.qnum === -2) {
						...
						return { 0: "e ", 1: "a ", 2: "b ", 3: "c ", 4: "d " }[cell.qnum];
					}
				} else if (cell.qnum === -2) {
					return "? ";
				} else if (cell.qnum >= 0) {
	...
	AnsCheck: {
		...
		checkDoubleNumberInBar: function() {
			...
			if (errcount !== this.failcode.length) {
				cells.setnoerr();
			}
		},
