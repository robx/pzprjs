...
(function(pidlist, classbase) {
	...
	KeyEvent: {
		...
		key_inputmark: function(ca) {
			...
			if (ca === " ") {
				...
				border.setQnum(border.qnum !== -2 ? -2 : -1);
			} else if ("0" <= ca && ca <= "9") {
				var num = +ca,
					cur = border.qnum;
				var max = border.maxnum();
				if (this.prev === border && cur > 0 && cur * 10 + num <= max) {
					num = cur * 10 + num;
				}
				if (num > 0 && num <= max) {
					border.setQnum(num);
	...
	"AreaNoriGraph:AreaGraphBase": {
		...
		isedgevalidbylinkobj: function(border) {
			return (
				border.sideobj[0].anum === border.sideobj[1].anum && !border.isBorder()
			);
		},
		isedgevalidbynodeobj: function(cell1, cell2) {
			return (
				cell1.anum === cell2.anum &&
				!this.board
					.getb((cell1.bx + cell2.bx) >> 1, (cell1.by + cell2.by) >> 1)
					.isBorder()
			);
		}
	},
	...
	FileIO: {
		...
		encodeData: function() {
			this.encodeBorder(function(border) {
				if (border.qnum >= 0) {
					return (border.ques === 1 ? "" : "!") + border.qnum + " ";
	...
	AnsCheck: {
		...
		checkSumOfNumber: function() {
			...
			for (var id = 0; id < boardborder.length; id++) {
				...
				var num1 = cell1.anum,
					num2 = cell2.anum;
				if (num1 === -1 || num2 === -1 || border.qnum === num1 + num2) {
					continue;
				}
		...
		checkOverSaturatedNumberInRoom: function() {
			...
			for (var r = 0; r < rooms.length; r++) {
				...
				var d = [],
					max = clist[0].getmaxnum(),
					min = clist[0].getminnum();
