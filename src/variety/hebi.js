...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		...
		inputModes: {
			...
			play: ["number", "dragnum+", "dragnum-", "objblank", "clear"]
		},
		mouseinput: function() {
			// オーバーライド
		...
		inputDot_snakes: function() {
			if (
				this.inputMode === "auto" &&
				(this.btn !== "right" ||
					(this.inputData !== null && this.inputData >= 0))
			) {
				return false;
			}
	...
	KeyEvent: {
		...
		keyinput: function(ca) {
			if (this.puzzle.editmode && this.key_inputdirec(ca)) {
				return;
			}

			if (this.puzzle.playmode && (ca === "q" || ca === "-")) {
				ca = "s1";
			}
	...
	AnsCheck: {
		...
		checkOtherAnsNumberInRoom: function() {
			this.checkDifferentNumberInRoom_main(
				this.board.snakemgr,
				this.isDifferentAnsNumberInClist
			);
		},
		...
		checkSideCell_snakes: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (!cell2.isnull && !!cell2.snake && cell.snake !== cell2.snake) {
					errcell = cell2;
				} else if (
					!cell3.isnull &&
					!!cell3.snake &&
					cell.snake !== cell3.snake
				) {
					errcell = cell3;
				}
				if (!!errcell) {
		...
		checkSnakesView: function() {
			...
			for (var r = 0; r < snakes.length; r++) {
				...
				var cell = clist.filter(function(cell) {
					return cell.anum === 1;
				})[0];
				if (!cell) {
					continue;
				...
				var pos = cell.getaddr(),
					clist2 = new this.klass.CellList();
				clist2.add(cell);
				while (!cell.isnull) {
					pos.movedir(dir, 2);
					cell = pos.getc();
					...
					if (cell.isnull || cell.qnum !== -1 || cell.anum !== -1) {
						break;
					}
				}
