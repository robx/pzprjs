...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		...
		mouseinput_other: function() {
			if (this.inputMode.indexOf("letter") === 0 && this.mousestart) {
				this.inputqchar_excell();
			}
		},
	...
	KeyEvent: {
		...
		key_inputexcell: function(ca) {
			...
			if ("0" <= ca && ca <= "9") {
				...
				if ((canum - 1) % 26 === num && canum > 0 && canum < 79) {
					excell.setQchar(canum + 26);
				} else if ((canum - 1) % 26 === num) {
					excell.setQchar(0);
				} else {
					excell.setQchar(num + 1);
				}
			} else if (ca === "-") {
				if (qn !== -1) {
					excell.setQnum(-1);
	...
	Board: {
		...
		searchLight: function(startexcell, setlight) {
			...
			while (dir !== 0) {
				...
				if (qb === 31) {
					if (dir === 1) {
						ldata[cc] = !isNaN({ 4: 1, 1: 1 }[ldata[cc]]) ? 1 : 2;
						dir = 3;
					} else if (dir === 2) {
						ldata[cc] = !isNaN({ 2: 1, 1: 1 }[ldata[cc]]) ? 1 : 4;
						dir = 4;
					} else if (dir === 3) {
						ldata[cc] = !isNaN({ 2: 1, 1: 1 }[ldata[cc]]) ? 1 : 4;
						dir = 1;
					...
					if (dir === 1) {
						ldata[cc] = !isNaN({ 5: 1, 1: 1 }[ldata[cc]]) ? 1 : 3;
						dir = 4;
					} else if (dir === 2) {
						ldata[cc] = !isNaN({ 3: 1, 1: 1 }[ldata[cc]]) ? 1 : 5;
						dir = 3;
					} else if (dir === 3) {
						ldata[cc] = !isNaN({ 5: 1, 1: 1 }[ldata[cc]]) ? 1 : 3;
						dir = 2;
			...
			if (!!setlight) {
				for (var c = 0; c < this.excell.length; c++) {
					this.excell[c].qinfo = 0;
				}
				startexcell.qinfo = 1;
	...
	Graphic: {
		...
		drawBGCells_kinkonkan: function() {
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (info === 1) {
					...
					this.drawTriangle1(px, py, cell.qinfo);
				} else {
					g.vhide();
				}
			}
		...
		getQuesNumberText: function(excell) {
			...
			if (canum === 0) {
				...
				text = this.getNumberTextCore_letter(canum <= 52 ? canum : canum - 52);
			}
			if (num >= 0) {
	...
	Encode: {
		...
		encodeKinkonkan: function() {
			...
			for (var ec = 0; ec < bd.excell.length; ec++) {
				var pstr = "",
					val = bd.excell[ec].qchar,
					qnum = bd.excell[ec].qnum;

				if (val > 0 && val <= 104) {
					if (val <= 26) {
						pstr = (val + 9).toString(36).toUpperCase();
					} else {
						pstr =
							(((val - 1) / 26 - 1) | 0).toString(10) +
							(((val - 1) % 26) + 10).toString(16).toUpperCase();
					}

					if (qnum === -2) {
						cm2 += ".";
					} else if (qnum < 16) {
						cm2 += "" + qnum.toString(16);
					} else {
						cm2 += "-" + qnum.toString(16);
					}
				} else {
	...
	AnsCheck: {
		...
		checkPairMirror: function() {
			this.checkMirrors(1, "pairedLetterNe");
		},
		checkReflectionCount: function() {
		...
		checkMirrors: function(type, code) {
			...
			for (var ec = 0; ec < bd.excell.length; ec++) {
				...
				if (
					...
					(type === 2 &&
						(excell.qnum !== excell2.qnum || excell.qnum !== ret.cnt))
				) {
					result = false;
					if (this.checkOnly) {
						break;
