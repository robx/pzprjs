...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		...
		mouseinput_auto: function() {
			if (this.puzzle.playmode) {
				...
				if (use === 1) {
					if (this.btn === "left") {
						if (this.mousestart) {
							this.inputTriangle_corner_start();
		...
		checkCornerData: function(cell) {
			...
			if (this.puzzle.getConfig("support_tri")) {
				...
				for (var key in adc) {
					...
					wall[key] =
						cell2.isWall() ||
						cell2.qans === data[key][0] ||
						cell2.qans === data[key][1];
					if (wall[key]) {
						++wall.count;
					}
				}
				...
				if (wall.count === 2) {
					if (wall.bottom && wall.left) {
						val = 2;
					} else if (wall.bottom && wall.right) {
						val = 3;
					} else if (wall.top && wall.right) {
						val = 4;
					} else if (wall.top && wall.left) {
						val = 5;
					} else {
						val = 0;
					}
		...
		inputTriangle_drag: function() {
			if (this.inputData === null || this.inputData <= 0) {
				return;
			}
			...
			var tri = this.checkCornerData(cell),
				ret = null,
				cur = this.inputData;
			if ((dbx === 2 && dby === 2) || (dbx === -2 && dby === -2)) {
				// 左上・右下
				if (cur === 2 || cur === 4) {
					ret = cur;
				}
			} else if ((dbx === 2 && dby === -2) || (dbx === -2 && dby === 2)) {
				// 右上・左下
				if (cur === 3 || cur === 5) {
					ret = cur;
				}
		...
		inputTriangle_onebtn: function() {
			...
			if (this.btn === "left") {
				this.inputData = [0, 2, 1, 3, 4, 5, -1][ans + 1];
			} else if (this.btn === "right") {
				this.inputData = [5, -1, 1, 0, 2, 3, 4][ans + 1];
			}
			cell.setAnswer(this.inputData);
			this.mouseCell = cell;
			cell.draw();
	...
	"AreaWrectGraph:AreaGraphBase": {
		...
		isedgevalidbynodeobj: function(cell1, cell2) {
			return (
				this.sldir[cell1.getdir(cell2, 2)][cell1.qans] &&
