...
(function(pidlist, classbase) {
	...
	"Graphic@arukone": {
		...
		drawChassis: function() {
			...
			var lw = this.lw,
				dw = 0.45 * this.bw,
				dh = 0.45 * this.bh;
			var boardWidth = bd.cols * this.cw - dw * 2,
				boardHeight = bd.rows * this.ch - dh * 2;
			g.fillStyle = "black";
			g.vid = "chs1_";
			g.fillRect(
				dw - (lw - 0.5),
			...
			g.vid = "chs3_";
			g.fillRect(dw - (lw - 0.5), dh - (lw - 0.5), boardWidth + 2 * lw - 2, lw);
			g.vid = "chs4_";
			g.fillRect(
				dw - (lw - 0.5),
				dh + boardHeight - 0.5,
		...
		drawBGCells: function() {
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (!!color) {
					g.fillStyle = color;
					var px1 =
						(cell.bx - (cell.bx > bd.minbx + 1 ? 1.5 : 0.6)) * this.bw - 0.5;
					var py1 =
						(cell.by - (cell.by > bd.minby + 1 ? 1.5 : 0.6)) * this.bw - 0.5;
					var px2 =
						(cell.bx + (cell.bx < bd.maxbx - 1 ? 1.5 : 0.6)) * this.bw + 0.5;
					var py2 =
						(cell.by + (cell.by < bd.maxby - 1 ? 1.5 : 0.6)) * this.bw + 0.5;
	...
	"FileIO@arukone": {
		...
		encodeConfig: function() {
			var disptype = this.puzzle.getConfig("dontpassallcell")
				? "dontpassallcell"
				: "passallcell";
			...
			this.encodeCell(function(cell) {
				...
				if (num > 0) {
					if (num > 0 && num <= 26) {
						...
						return (num - 17).toString(36).toLowerCase() + " ";
					}
				} else if (num === -2) {
					return "- ";
				} else {
	...
	AnsCheck: {
		...
		checkNoLine_arukone: function() {
			if (!this.puzzle.getConfig("dontpassallcell")) {
				this.checkNoLine();
			}
		}
	}
