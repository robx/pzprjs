	...
	Encode: {
		...
		encodeBorder_if_exist: function() {
			for (var id = 0; id < this.board.border.length; id++) {
				if (this.board.border[id].ques === 1) {
					this.encodeBorder();
					break;
				}
			}
	...
	AnsCheck: {
		...
		checkArrowNumber_tatami: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				var bx = cell.bx,
					...
					blist;
				if (dir === cell.UP) {
					blist = bd.borderinside(bx, bd.minby, bx, by);
				} else if (dir === cell.DN) {
					blist = bd.borderinside(bx, by, bx, bd.maxby);
				} else if (dir === cell.LT) {
					blist = bd.borderinside(bd.minbx, by, bx, by);
				} else if (dir === cell.RT) {
					blist = bd.borderinside(bx, by, bd.maxbx, by);
				} else {
					continue;
				}
				if (
					cell.qnum ===
					blist.filter(function(border) {
		...
		checkArrowNumber_border: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				var cell = bd.cell[c],
					dir = cell.qdir;
				if (!cell.isValidNum() || !dir || cell.reldirbd(dir, 1).isBorder()) {
					continue;
				}
