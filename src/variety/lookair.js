...
(function(pidlist, classbase) {
	...
	Cell: {
		...
		countDir5ShadedCell: function() {
			...
			for (var i = 0; i < 5; i++) {
				if (!cells[i].isnull && cells[i].isShade()) {
					cnt++;
				}
			}
			return cnt;
	...
	AnsCheck: {
		...
		checkDir5ShadeCell: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (
					!cell.isValidNum() ||
					cell.getNum() === cell.countDir5ShadedCell()
				) {
					continue;
				}
		...
		checkLookair: function() {
			...
			function subcheck(base, bx, by) {
				...
				if (cell.isnull) {
					return 1;
				} /* break with no error (end of board) */ else if (!cell.isShade()) {
					return 0;
				} /* continue loop */
			...
			allloop: for (var r = 0; r < bd.sblkmgr.components.length; r++) {
				var base = bd.sblkmgr.components[r].clist,
					d = base.getRectSize();
