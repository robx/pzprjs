...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		...
		inputqans: function() {
			...
			if (!bcell.isnull) {
				var tmp,
					d = { x1: cell.bx, y1: cell.by, x2: bcell.bx, y2: bcell.by };
				...
				for (var bx = d.x1; bx <= d.x2; bx += 2) {
					for (var by = d.y1; by <= d.y2; by += 2) {
						...
						if (!cell2.isnull && cell2.isStone()) {
							if (cell2.anum === -1 || (max >= 2 && cell2.anum === max - 1)) {
								return;
							}
						}
					}
	...
	// 正解判定処理実行部
	AnsCheck: {
		checklist: ["checkStoneExist", "checkPickedStone"],
