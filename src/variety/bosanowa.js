...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		inputModes: {
			edit: ["number", "clear"],
			play: ["number", "subnumber", "subnumber-", "clear"]
		},
		mouseinput_clear: function() {
			this.inputclean_all();
		},
		mouseinput_other: function() {
			if (this.inputMode.indexOf("subnumber") === 0 && this.mousestart) {
				this.inputsubnumber_border();
			}
		},
		...
		inputsubnumber_border: function() {
			var pos = this.getpos(0.31),
				border = pos.getb();
			if (border.isnull || !border.isGrid()) {
				return;
			}
		...
		key_inputqnum_bosanowa: function(ca) {
			...
			if (cursor.oncell()) {
				...
				if ("0" <= ca && ca <= "9") {
					...
					if (qs <= 0 || this.prev !== border) {
						if (num <= qsubmax) {
							border.setQsub(num);
						}
					} else {
						if (qs * 10 + num <= qsubmax) {
							border.setQsub(qs * 10 + num);
						} else if (num <= qsubmax) {
							border.setQsub(num);
						}
					}
	...
	Graphic: {
		...
		drawGrid_waritai: function() {
			...
			var csize = this.cw * 0.2;
			var blist = this.range.borders;
			for (var i = 0; i < blist.length; i++) {
				var border = blist[i],
					isgrid = border.isGrid();
				var px = border.bx * this.bw,
					py = border.by * this.bh;
				...
				if (isgrid) {
					...
					if (border.isVert()) {
						g.fillRectCenter(px, py, 0.5, csize);
					} else {
						g.fillRectCenter(px, py, csize, 0.5);
					}
				} else {
					g.vhide();
				}
		...
		drawOutside_souko: function() {
			...
			for (var bx = (d.x1 - 2) | 1; bx <= d.x2 + 2; bx += 2) {
				for (var by = (d.y1 - 2) | 1; by <= d.y2 + 2; by += 2) {
					...
					if (
						addr.getc().isEmpty() &&
						(addr.relcell(-2, 0).ques === 0 ||
							addr.relcell(2, 0).ques === 0 ||
							addr.relcell(0, -2).ques === 0 ||
							addr.relcell(0, 2).ques === 0 ||
							addr.relcell(-2, -2).ques === 0 ||
							addr.relcell(2, -2).ques === 0 ||
							addr.relcell(-2, 2).ques === 0 ||
							addr.relcell(2, 2).ques === 0)
					) {
	...
	Encode: {
		decodePzpr: function(type) {
			...
			if (this.checkpflag("h")) {
				puzzle.setConfig("disptype_bosanowa", 2);
			} else if (this.checkpflag("t")) {
				puzzle.setConfig("disptype_bosanowa", 3);
			}
		},
		encodePzpr: function(type) {
			...
			var puzzle = this.puzzle;
			if (puzzle.getConfig("disptype_bosanowa") === 2) {
				this.outpflag = "h";
			} else if (puzzle.getConfig("disptype_bosanowa") === 3) {
				this.outpflag = "t";
			}
		},
	...
	// 正解判定処理実行部
	AnsCheck: {
		checklist: ["checkSubsNumber", "checkNoNumCell+"],
		...
		checkSubsNumber: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				sub = subs[adb.bottom.id];
				if (sub > 0) {
					sum += sub;
				} else if (sub === UNDEF) {
					continue;
				}
				sub = subs[adb.left.id];
				if (sub > 0) {
					sum += sub;
				} else if (sub === UNDEF) {
					continue;
				}
				sub = subs[adb.right.id];
				if (sub > 0) {
					sum += sub;
				} else if (sub === UNDEF) {
					continue;
