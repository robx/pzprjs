...
KeyEvent:{
	...
	key_inputdot : function(ca){
		...
		if(dot!==null){
			if     (ca==='1'){ dot.setDot(2);}
			else if(ca===' '||ca==='-'||ca==='0'){ dot.setDot(0);}
			else { return;}
			dot.draw();
		}
	}
...
Dot:{
	setDot : function(val){
		if(this.piece.group==="cross" && val===2) { return; }
...
AnsCheck:{
	...
	checkDotLength: function(){
		...
		for(var s=0;s<bd.dotsmax;s++){
			...
			if(top.length===bottom.length && left.length===right.length) {continue;}
	...
	checkDotOnLine : function(){
		...
		for(var s=0;s<bd.dotsmax;s++){
			...
			if(dot.piece.group==="cell" && dot.piece.lcnt > 0) { continue; }
			if(dot.piece.group==="border" && dot.piece.line > 0) { continue; }
...
FailCode:{
	lnOnDot    : ["(please translate) A line doesn't go over a circle.","A line doesn't go over a circle."],
	lnOnCenter : ["(please translate) A circle is not the center point of a line.","A circle is not the center point of a line."]
}
