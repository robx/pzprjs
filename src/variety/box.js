...
(function(pidlist, classbase) {
	...
	Graphic: {
		...
		drawCircledNumbers_box: function() {
			...
			if (x2 >= bd.maxbx) {
				for (
					var by = y1 | 1, max = Math.min(bd.maxby, y2);
					by <= max;
					by += 2
				) {
					list.push([bd.maxbx + 1, by]);
				}
			}
			if (y2 >= bd.maxby) {
				for (
					var bx = x1 | 1, max = Math.min(bd.maxbx, x2);
					bx <= max;
					bx += 2
				) {
					list.push([bx, bd.maxby + 1]);
				}
			}
			...
			for (var i = 0; i < list.length; i++) {
				var num =
					((list[i][0] !== bd.maxbx + 1 ? list[i][0] : list[i][1]) + 1) >> 1;
		...
		decodeBox: function() {
			...
			for (var a = 0; a < bstr.length; a++) {
				var ca = bstr.charAt(a),
					excell = bd.excell[ec];
				if (ca === "-") {
					excell.qnum = parseInt(bstr.substr(a + 1, 2), 32);
					a += 2;
				} else {
					excell.qnum = parseInt(ca, 32);
				}
				ec++;
				if (ec >= bd.cols + bd.rows) {
					a++;
					break;
				}
			}
	...
	// 正解判定処理実行部
	AnsCheck: {
		checklist: ["checkShadeCellExist", "checkShadeCells"],
