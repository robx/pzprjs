...
"Room:BoardPiece":{
	...
	isRoom : function(){
		if(this.top.isnull){ return false;}
	...
	countConn : function(){
		...
		var c1=this.top, c2=this.getBottomCell(), bd=this.board;
		var corners = [bd.getx(c1.bx-1,c1.by-1), bd.getx(c2.bx+1,c1.by-1), bd.getx(c1.bx-1,c2.by+1), bd.getx(c2.bx+1,c2.by+1)];
		var count=0;
		for(var i=0;i<corners.length;i++){
			if(corners[i].isCheckerboard()){ count++;}
		}
		return count;
	}
},
Cross:{
	isCheckerboard : function(){
		...
		if(bx<=bd.minbx||bx>=bd.maxbx||by<=bd.minby||by>=bd.maxby){ return false;}
		var adj=[bd.getc(bx-1,by-1), bd.getc(bx-1,by+1), bd.getc(bx+1,by+1),bd.getc(bx+1,by-1)];
		var sh=[adj[0].isShade(), adj[1].isShade(), adj[2].isShade(), adj[3].isShade()];
		if(sh[0]&&sh[2]&&!sh[1]&&!sh[3]){ return true;}
		if(!sh[0]&&!sh[2]&&sh[1]&&sh[3]){ return true;}
		return false;
	}
},
	...
	get8clist : function(){
		...
		for(var dx=-2; dx<=2; dx++){
			for(var dy=-2; dy<=2; dy++){
				...
				if(c.isnull||c.group!=="cell"){ continue;}
				list.push(c);
			}
		}
		return list;
...
'AreaDiagShadeGraph:AreaGraphBase':{
	...
	setComponentRefs : function(obj, component){ obj.sdblk = component;},
	getObjNodeList   : function(nodeobj){ return nodeobj.sdblknodes;},
	resetObjNodeList : function(nodeobj){ nodeobj.sdblknodes = [];},

	isnodevalid : function(cell){ return cell.isShade();},
	isedgevalidbynodeobj : function(cell1, cell2){ return true;},
	getSideObjByNodeObj : function(cell){
		var list = cell.get8clist(), cells = [];
...
Graphic:{
	...
	getLineColor : function(border){
		if(!border.isLine()){ return;}
		if(border.sidecell[0].error||border.sidecell[1].error){ return this.errlinecolor;}
		if(border.sidecell[0].trial||border.sidecell[1].trial){ return this.triallinecolor;}
...
AnsCheck:{
	...
	checkRoomRect : function(){
		this.checkAllRoom( function(room){ return (!room.isRectangle());}, "bkNotRect");
	},
	...
	checkAllRoom : function(func, code){
		for(var i=0;i<this.board.room.length;i++){
			...
			if(!room.isRoom()){ continue;}
			if(!func(room)){ continue;}
...
FailCode:{
	bkNumUnshade : ["長方形の中に含まれていない丸があります。","A clue is not inside a rectangle."],
	...
	bkNotRect : ["四角形ではない領域があります。","An area is not a rectangle."],
	csdDivide : ["長方形で出来た輪っかに含まれていない長方形があります。","There is a rectangle that is not connected to the other rectangles."],
	rmBranch : ["３つ以上の頂点を共有している長方形があります。","There is a branch of rectangles."],
	rmDeadend : ["頂点を１つしか共有していない長方形があります。","There is a dead-end rectangle."],
	rmIsolated : ["頂点を全く共有していない長方形があります。","There is an isolated rectangle."],
	rmRectUnshade : ["輪の中に長方形が出来ています。","A rectangle is not part of the solution."]
}
}));
