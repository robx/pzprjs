...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		...
		inputGateNumber: function(cell) {
			var bd = this.board;
			if (this.btn === "left") {
				cell.setQues({ 0: 1, 1: 21, 21: 22, 22: 0 }[cell.ques]);
			} else if (this.btn === "right") {
				cell.setQues({ 0: 22, 22: 21, 21: 1, 1: 0 }[cell.ques]);
			}
			cell.setNum(-1);
			bd.gatemgr.rebuild();
		...
		key_inputqnum_slalom: function(ca) {
			...
			if (
				...
				if (newques !== -1) {
					...
					if (old === 21 || old === 22 || newques === 21 || newques === 22) {
						bd.gatemgr.rebuild();
					}
	...
	Cell: {
		...
		posthook: {
			qnum: function(num) {
				this.board.gatemgr.generateGateNumberAll();
			},
			qdir: function(num) {
				this.board.gatemgr.generateGateNumberAll();
			}
		...
		getConnectingGate: function() {
			var adc = this.adjacent,
				...
				gatelist = [];
			cell2 = adc.top;
			if (!cell2.isnull && cell2.ques === 21) {
				gatelist.push(cell2.gate);
			}
			cell2 = adc.bottom;
			if (!cell2.isnull && cell2.ques === 21) {
				gatelist.push(cell2.gate);
	...
	"StartPosAddress:Address": {
		...
		addOpe: function(bx, by) {
			if (!this.puzzle.ready || (this.bx === bx && this.by === by)) {
				return;
			}
			this.puzzle.opemgr.add(
				new this.klass.StartposOperation(this.bx, this.by, bx, by)
			);
		}
	},
	...
	"AreaHurdleGraph:AreaGraphBase": {
		...
		isnodevalid: function(cell) {
			return cell.ques === 21 || cell.ques === 22;
		},
		isedgevalidbynodeobj: function(cell1, cell2) {
			var dir = cell1.getdir(cell2, 2);
			if (dir === cell1.UP || dir === cell1.DN) {
				return cell1.ques === 21 && cell2.ques === 21;
			} else if (dir === cell1.LT || dir === cell1.RT) {
				return cell1.ques === 22 && cell2.ques === 22;
			}
			return false;
		},
		...
		generateGateNumberAll: function() {
			...
			var decnumber = {
				...
				erase: function() {
					...
					for (var r = 0; r < gates.length; r++) {
						this.nums[r] = this.nums[r].filter(function(num) {
							return !doneobj[num];
						});
					}
				}
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (cell.ques === 1) {
					...
					if ((dir === cell.NDIR || dir === cell.UP) && adc.top.ques === 21) {
						decnumber.nums[gates.indexOf(adc.top.gate)].push(qn);
					}
					if (
						(dir === cell.NDIR || dir === cell.DN) &&
						adc.bottom.ques === 21
					) {
						decnumber.nums[gates.indexOf(adc.bottom.gate)].push(qn);
					}
					if ((dir === cell.NDIR || dir === cell.LT) && adc.left.ques === 22) {
						decnumber.nums[gates.indexOf(adc.left.gate)].push(qn);
					}
					if ((dir === cell.NDIR || dir === cell.RT) && adc.right.ques === 22) {
						decnumber.nums[gates.indexOf(adc.right.gate)].push(qn);
					}
				}
			}
			...
			while (repeatflag) {
				...
				for (var r = 0; r < gates.length; r++) {
					...
					var gate = gates[r],
						nums = decnumber.nums[r];
					var cand = nums.length === 1 ? nums[0] : -1;
					if (cand > 0 && numcnt[cand] > 1) {
						cand = -1;
					}
				...
				for (var r = 0; r < gates.length; r++) {
					decnumber.nums[r].forEach(function(num) {
						numcnt[num]++;
					});
				}
		...
		setGateError: function(gate, val) {
			...
			if (!cell1.isnull && cell1.ques === 1) {
				clist.add(cell1);
			}
			if (!cell2.isnull && cell2.ques === 1) {
				clist.add(cell2);
			}
			clist.seterr(val);
		}
	...
	Graphic: {
		...
		drawGates: function() {
			...
			for (var i = 0; i < clist.length; i++) {
				...
				if (cell.ques === 22) {
					//よこ
					var px,
						py = cell.by * this.bh - lm + 1,
						rx = (cell.bx - 1) * this.bw,
						max = rx + this.cw;
					g.beginPath();
		...
		drawStartpos: function() {
			...
			g.fillStyle =
				this.puzzle.mouse.inputData === 10 ? this.errbcolor1 : "white";
		...
		repaintParts: function(blist) {
			...
			for (var i = 0; i < clist.length; i++) {
				var cell = clist[i];
				if (!this.board.startpos.equals(cell)) {
					continue;
				}
	...
	Encode: {
		...
		decodeSlalom: function(ver) {
			...
			for (i = 0; i < array[0].length; i++) {
				...
				if (ca === "1") {
					...
					cell.ques = 22;
				} else if (this.include(ca, "4", "9") || this.include(ca, "a", "z")) {
					c += parseInt(ca, 36) - 4;
				}
			...
			if (ver === 0) {
				...
				for (i = i + 1; i < array[0].length; i++) {
					...
					if (this.include(ca, "0", "9") || this.include(ca, "a", "f")) {
						...
						bd.gatemgr.components[r].number = parseInt(
							bstr.substr(i + 1, 2),
							16
						);
						i += 2;
					...
					r++;
					if (r > bd.gatemgr.components.length) {
						break;
					}
				}

				for (var c = 0; c < bd.cell.length; c++) {
					var gatelist = bd.cell[c].getConnectingGate(),
						min = 1000;
				...
				for (i = i + 1; i < array[0].length; i++) {
					...
					if (cell.ques !== 1) {
						...
						if (
							ver === 1 &&
							(this.include(ca, "0", "9") || this.include(ca, "a", "f"))
						) {
		...
		encodeSlalom: function(ver) {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				...
				if (count === 0) {
					cm += pstr;
				} else if (pstr || count === 32) {
					cm += (3 + count).toString(36) + pstr;
					count = 0;
				}
			}
			if (count > 0) {
				cm += (3 + count).toString(36);
			}
			...
			if (ver === 0) {
				...
				for (var c = 0; c < bd.cell.length; c++) {
					...
					var pstr = "",
						...
						dir = cell.qdir;
					if (val >= 1 && val < 16) {
						pstr = (ver === 1 ? "" : "" + dir) + val.toString(16);
					} else if (val >= 16 && val < 256) {
						pstr = (ver === 1 ? "-" : "" + (dir + 5)) + val.toString(16);
					} else if (val >= 256 && val < 4096) {
						pstr = "-" + dir + val.toString(16);
					} else {
						count++;
					}
		...
		encodeBoard_pzpr2: function() {
			...
			this.encodeCell(function(cell) {
				if (bd.startpos.equals(cell)) {
					...
					return "- ";
				} else if (cell.ques === 1 && cell.qnum <= 0) {
					return "# ";
				} else if (cell.ques === 1) {
		...
		decodeBoard_old: function() {
			...
			this.decodeCell(function(cell, ca) {
				...
				if (ca === "#") {
					...
					if (ca.charAt(0) === "i") {
						cell.ques = 21;
					} else if (ca.charAt(0) === "w") {
						cell.ques = 22;
					}
					if (ca.length > 1) {
						sv_num[c] = +ca.substr(1);
					}
				}
			});
			bd.gatemgr.rebuild();
			...
			for (var c = 0; c < bd.cell.length; c++) {
				if (sv_num[c] !== -1) {
					bd.cell[c].gate.number = sv_num[c];
				}
			}
			for (var c = 0; c < bd.cell.length; c++) {
				var gatelist = bd.cell[c].getConnectingGate(),
	...
	AnsCheck: {
		...
		checkPassGateOnce: function() {
			return this.checkGateLine(1, "gateRedup");
		},
		checkPassAllGate: function() {
			return this.checkGateLine(2, "gateUnpass");
		},
		checkGateLine: function(type, code) {
			var bd = this.board,
				gates = bd.gatemgr.components;
			for (var r = 0; r < gates.length; r++) {
				var cnt = 0,
					clist = gates[r].clist;
				for (var i = 0; i < clist.length; i++) {
		...
		searchTraceInfo: function(cell1) {
			...
			var pos = cell1.getaddr(),
				passed = 0,
				ordertype = -1,
				gatecount = this.board.gatemgr.components.length;
					...
					if (cell.ques === 21 || cell.ques === 22) {
						...
						if (gatenumber <= 0) {
							...
							if (gatenumber === revgatenumber) {
								if (passed !== gatenumber) {
									errgate = gate;
									break;
								}
							} else if (passed === gatenumber) {
								ordertype = 1;
							} // 順方向と確定
							else if (passed === revgatenumber) {
								// 逆方向なので別の方向から回る
								if (cell1.lcnt < 2) {
									break;
								} // 1つしか線がない場合は終了
						...
						} else if (ordertype === 1 && passed !== gatenumber) {
							errgate = gate;
							break;
						} // 通過順間違い
					}
