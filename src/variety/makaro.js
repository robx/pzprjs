...
(function(pidlist, classbase) {
	...
	MouseEvent: {
		inputModes: {
			edit: ["border", "arrow", "number", "clear"],
			play: ["number", "clear"]
		},
	...
	KeyEvent: {
		...
		keyinput: function(ca) {
			...
			if (this.puzzle.editmode) {
				if (this.key_inputcell_makaro_edit(cell, ca)) {
					return;
				}
			}
	...
	Border: {
		isBorder: function() {
			return (
				this.isnull ||
				this.ques > 0 ||
				!!(this.sidecell[0].ques === 1 || this.sidecell[1].ques === 1)
			);
		}
	},
	Board: {
		...
		decodeMakaro: function() {
			...
			for (i = 0; i < bstr.length; i++) {
				...
				if (this.include(ca, "0", "9")) {
					cell.qnum = parseInt(ca, 10) + 1;
				} else if (ca === "-") {
					cell.qnum = parseInt(bstr.substr(i + 1, 2), 10) + 1;
					i += 2;
				} else if (ca >= "a" && ca <= "e") {
					cell.ques = 1;
					cell.qdir = parseInt(ca, 36) - 10;
				} else if (ca >= "g" && ca <= "z") {
					c += parseInt(ca, 36) - 16;
				}
		...
		encodeMakaro: function() {
			...
			for (var c = 0; c < bd.cell.length; c++) {
				var pstr = "",
					...
					qn = cell.qnum;
				if (qn >= 1 && qn < 11) {
					pstr = (qn - 1).toString(10);
				} else if (qn >= 11 && qn < 100) {
					pstr = "-" + (qn - 1).toString(10);
				} else if (cell.ques === 1) {
					pstr = (cell.qdir + 10).toString(36);
				} else {
					count++;
				}
