	...
	Board: {
		...
		getBorderLengthInfo: function() {
			...
			for (var i = 0; i < this.border.length; i++) {
				rdata.id[i] = this.border[i].isBorder() ? 0 : null;
			}
			for (var i = 0; i < this.border.length; i++) {
				var border0 = this.border[i];
				if (rdata.id[border0.id] !== 0) {
					continue;
				}
				var path = rdata.addPath();
				var pos = border0.getaddr(),
					isvert = border0.isVert(),
					n = 0;
				while (1) {
					var border = pos.getb();
					if (border.isnull || rdata.id[border.id] !== 0) {
						break;
					}
