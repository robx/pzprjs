	...
	"MouseEvent@minarism": {
		...
		inputmark_mouseup: function() {
			...
			if (!this.cursor.equals(pos) && this.inputMode === "auto") {
				...
				if (val >= 0) {
					border.setQdir(0);
					border.setQnum(val >= 1 ? val : -2);
				} else if (val === -1) {
					border.setQdir(0);
					border.setQnum(-1);
				} else {
					border.setQdir(val + (border.bx % 2 === 1 ? 4 : 6));
					border.setQnum(-1);
				}
				border.draw();
			}
		...
		inputmark_kropki: function() {
			...
			if (!this.cursor.equals(pos) && this.inputMode === "auto") {
				...
				if (this.inputMode === "circle-unshade") {
					border.setQnum(border.qnum !== 1 ? 1 : 0);
				} else if (this.inputMode === "circle-shade") {
					border.setQnum(border.qnum !== 2 ? 2 : 0);
	...
	KeyEvent: {
		...
		key_inputmark: function(ca) {
			...
			if (this.pid === "kropki") {
				...
				var tmp = border.NDIR;
				if (ca === "q") {
					tmp = border.isHorz() ? border.UP : border.LT;
				}
				if (ca === "w") {
					tmp = border.isHorz() ? border.DN : border.RT;
				}
				...
				if (cur <= 0 || this.prev !== border) {
					if (num <= max) {
						border.setQnum(num);
					}
				} else {
					if (cur * 10 + num <= max) {
						border.setQnum(cur * 10 + num);
					} else if (num <= max) {
						border.setQnum(num);
					}
				}
		...
		decodeMinarism: function(type) {
			...
			for (var i = 0; i < bstr.length; i++) {
				...
				if (this.include(ca, "0", "9") || this.include(ca, "a", "f")) {
					border.qnum = parseInt(ca, 16);
				} else if (ca === "-") {
					border.qnum = parseInt(bstr.substr(i + 1, 2), 16);
					i += 2;
				} else if (ca === ".") {
					border.qnum = -2;
				} else if (ca === "g") {
					tmp = type === parser.URL_PZPRV3 || id < bd.cols * bd.rows ? 1 : 2;
				} else if (ca === "h") {
					tmp = type === parser.URL_PZPRV3 || id < bd.cols * bd.rows ? 2 : 1;
				} else if (this.include(ca, "i", "z")) {
					id += parseInt(ca, 36) - 18;
				} else if (type === parser.URL_PZPRAPP && ca === "/") {
					id = bd.cell.length - 1;
				}
			...
			for (var id = 0; id < bd.border.length; id++) {
				...
				if (!!border) {
					...
					if (dir === border.UP || dir === border.LT) {
						pstr = type === parser.URL_PZPRV3 || !!bd.cell[id] ? "g" : "h";
					} else if (dir === border.DN || dir === border.RT) {
						pstr = type === parser.URL_PZPRV3 || !!bd.cell[id] ? "h" : "g";
					} else if (qnum === -2) {
						pstr = ".";
					} else if (qnum >= 0 && qnum < 16) {
						pstr = "" + qnum.toString(16);
					} else if (qnum >= 16 && qnum < 256) {
						pstr = "-" + qnum.toString(16);
					} else {
						count++;
					}
	...
	"Encode@kropki": {
		...
		decodeCircle_kropki: function(type) {
			...
			var bstr = this.outbstr,
				...
				tri = [9, 3, 1];
			var pos = bstr
				? Math.min(((bd.border.length + 2) / 3) | 0, bstr.length)
				: 0;
			for (var i = 0; i < pos; i++) {
		...
		encodeCircle_kropki: function(type) {
			...
			for (var c = 0; c < bd.border.length; c++) {
				if (bd.border[c].qnum > 0) {
					pass += bd.border[c].qnum * tri[num];
				}
				num++;
				if (num === 3) {
					cm += pass.toString(27);
		...
		encodeData: function() {
			this.encodeBorder(function(border) {
				...
				if (dir === border.UP || dir === border.LT) {
					...
					return "b ";
				} else if (border.qnum === -2) {
					return ". ";
				} else if (border.qnum !== -1) {
					return border.qnum + " ";
				} else {
					return "0 ";
				}
