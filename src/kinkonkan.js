//
// パズル固有スクリプト部 キンコンカン版 tilepaint.js v3.1.9p1
//
...
Puzzle.prototype = {
	...
	check : function(){
		...
		if( !ans.checkOneNumber(rarea, function(top,lcnt){ return (lcnt>1);}, function(cc){ return bd.getQansCell(cc)>0;}) ){
			ans.setAlert('斜線が複数引かれた部屋があります。', 'A room has plural mirrors.'); return false;
		}

		if( !this.checkMirrors(1) ){
			ans.setAlert('光が同じ文字の場所へ到達しません。', 'Beam from a light doesn\'t reach one\'s pair.'); return false;
		}

		if( !this.checkMirrors(2) ){
			ans.setAlert('光の反射回数が正しくありません。', 'The count of refrection is wrong.'); return false;
		}

		if( !ans.checkOneNumber(rarea, function(top,lcnt){ return (lcnt==0);}, function(cc){ return bd.getQansCell(cc)>0;}) ){
			ans.setAlert('斜線の引かれていない部屋があります。', 'A room has no mirrors.'); return false;
		}
