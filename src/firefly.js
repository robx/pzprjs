//
// パズル固有スクリプト部 ホタルビーム版 firefly.js v3.2.0
//
	...
	answer_init : function(){
		...
		ans.checkLcntCell = function(val){
			...
			for(var c=0;c<bd.cell.length;c++){
				if(bd.QnC(c)==-1 && ans.lcnts.cell[c]==val){
		...
		ans.checkFireflyBeam = function(val){
			if(ans.lcnts.total[val]==0){ return true;}
			for(var c=0;c<bd.cell.length;c++){
				if(bd.QnC(c)!=-1 && bd.DiC(c)!=0){
					if((bd.DiC(c)==1 && bd.LiB(bd.ub(c))!=1) || (bd.DiC(c)==2 && bd.LiB(bd.db(c))!=1) ||
					   (bd.DiC(c)==3 && bd.LiB(bd.lb(c))!=1) || (bd.DiC(c)==4 && bd.LiB(bd.rb(c))!=1) )
					{
						bd.sErC([c],1);
						return false;
					}
		...
		ans.checkStrangeLine = function(saved){
			...
			for(var id=0;id<bd.border.length;id++){
				if(bd.LiB(id)==1 && saved.area.check[id]!=2){ idlist.push(id);}
			}
			if(idlist.length>0){
				bd.sErB(bd.borders,2);
				bd.sErB(idlist,1);
