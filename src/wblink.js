//
// パズル固有スクリプト部 シロクロリンク版 wblink.js v3.1.9p2
//
...
Puzzle.prototype = {
	...
	input_init : function(){
		...
		mv.getidlist = function(id){
			...
			if(bd.border[id].cx%2==1){
				while(cy>=0         && bd.getQuesCell(bd.getcnum(cx,cy  ))==0){ cy--;} by1=2*cy+2;
				while(cy<=k.qrows-1 && bd.getQuesCell(bd.getcnum(cx,cy+1))==0){ cy++;} by2=2*cy+2;
				bx1 = bx2 = bd.border[id].cx;
			}
			else if(bd.border[id].cy%2==1){
				while(cx>=0         && bd.getQuesCell(bd.getcnum(cx  ,cy))==0){ cx--;} bx1=2*cx+2;
				while(cx<=k.qcols-1 && bd.getQuesCell(bd.getcnum(cx+1,cy))==0){ cx++;} bx2=2*cx+2;
		...
		kc.input41_42 = function(ca){
			...
			if     (ca=='1'){ bd.setQuesCell(cc,(bd.getQuesCell(cc)!=41?41:0)); flag = true;}
			else if(ca=='2'){ bd.setQuesCell(cc,(bd.getQuesCell(cc)!=42?42:0)); flag = true;}
			else if(ca=='-'){ bd.setQuesCell(cc,(bd.getQuesCell(cc)!=-2?-2:0)); flag = true;}
			else if(ca=='3'||ca==" "){ bd.setQuesCell(cc,0); flag = true;}
			...
			for(var i=0;i<clist.length;i++){
				...
				if(bd.getQuesCell(c)==41 || bd.getQuesCell(c)==42){
					...
					if(this.vnop("c"+c+"_cir41a_",1)){
						g.beginPath(); g.arc(bd.cell[c].px()+int(k.cwidth/2), bd.cell[c].py()+int(k.cheight/2), rsize , 0, Math.PI*2, false); g.fill();
					}
				}
				...
				if(bd.getQuesCell(c)==41){
					...
					if(this.vnop("c"+c+"_cir41b_",1)){
						g.beginPath(); g.arc(bd.cell[c].px()+int(k.cwidth/2), bd.cell[c].py()+int(k.cheight/2), rsize2, 0, Math.PI*2, false); g.fill();
					}
				}
	...
	check : function(){
		...
		if( !ans.checkOneNumber(larea, function(a,cnt){ return (cnt>=3);}, function(c){ return (bd.getQuesCell(c)!=0);} ) ){
			ans.setAlert('3つ以上の○が繋がっています。','Three or more objects are connected.'); return false;
		}

		if( !this.checkWBcircle(larea, 41) ){
			ans.setAlert('白丸同士が繋がっています。','Two white circles are connected.'); return false;
		}
		if( !this.checkWBcircle(larea, 42) ){
			ans.setAlert('黒丸同士が繋がっています。','Two black circles are connected.'); return false;
		}

		if( !ans.checkAllCell(function(c){ return (bd.getQuesCell(c)!=0 && ans.lcntCell(c)==0);} ) ){
	...
	checkWBcircle : function(larea,val){
		for(var r=1;r<=larea.max;r++){
			var tip1=-1, tip2=-1;
			if(larea.room[r].length>1){
				var tip1 = larea.room[r][0];
				var tip2 = larea.room[r][larea.room[r].length-1];
				if(bd.getQuesCell(tip1)==val && bd.getQuesCell(tip2)==val){
					bd.setErrorBorder(bd.borders,2); ans.setErrLareaById(larea,r,1);
					bd.setErrorCell([tip1,tip2],1);
					return false;
				}
			}
		}
